...
public class ProjectSecurityConfig {
	 ...
	 * admin = User.withDefaultPasswordEncoder() .username("admin")
	...
	/*
	 * @Bean public InMemoryUserDetailsManager userDetailsService() { UserDetails
	 * admin = User.withUsername("admin") .password("12345") .authorities("admin")
	 * .build(); UserDetails user = User.withUsername("user") .password("1234")
	 * .authorities("read") .build(); return new InMemoryUserDetailsManager(admin,
	 * user); }
	 */

	/*
	 * public UserDetailsService userDetailsService(DataSource dataSource) { return
